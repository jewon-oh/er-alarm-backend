...
public class ApiResponse<T> {
    ...
    public static <T> ResponseEntity<ApiResponse<T>> ok() {
        return ResponseEntity.ok(new ApiResponse<>(true, null, "요청 성공", null,null));
    }

    public static <T> ResponseEntity<ApiResponse<T>> ok(T data) {
        return ResponseEntity.ok( new ApiResponse<>(true, null, "요청 성공", data,null));
    }

    public static <T> ResponseEntity<ApiResponse<T>> ok(T data,String message) {
        return  ResponseEntity.ok(new ApiResponse<>(true, null, message, data,null));
    }

    public static <T> ResponseEntity<ApiResponse<T>> fail(String code, String message) {
        return ResponseEntity.ok(new ApiResponse<>(false,code,message,null,null));
    }

    public static <T> ResponseEntity<ApiResponse<T>> error(HttpStatusCode status, String code, String message) {
        return ResponseEntity.status(status).body(new ApiResponse<>(false, code, message, null,null));
    }
    ...
    public static <T> ResponseEntity<ApiResponse<T>> error(HttpStatusCode status, String code, String message, List<ErrorDetail> details) {
        return ResponseEntity.status(status).body(new ApiResponse<>(false, code, message, null, details));
    }
